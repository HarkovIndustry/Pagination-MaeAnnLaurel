document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderPagination(totalItems) {
    ...
    const prevButton = `<li class="page-item">
                              <a class="page-link prev" href="#" aria-label="Previous" data-page="prev">
                                  Previous
                              </a>
                          </li>`;
    ...
    const nextButton = `<li class="page-item">
                              <a class="page-link next" href="#" aria-label="Next" data-page="next">
                                  Next
                              </a>
                          </li>`;
    ...
    paginatedItems.forEach((detail) => {
      const images = detail.images
        .map((image) => {
          ...
          return `<div class="col-6 col-md-4 col-lg-2 mb-4 img-hover"><img src="${source}" alt="${detail.place}" class="img-thumbnail w-100 rounded-circle img-hover"></div>`;
        })
        .join("");

      const infoContent = `
              <h2 class="text-center">${detail.place}</h2>
              <div class="row">${images}</div>
              <p class="text-justify">${detail.description}</p>
              <iframe src="${
                detail.buttonDirectory.address.map
              }" style="width: 100%; height: 450px; border: 0;" allowfullscreen="" loading="lazy"></iframe>
              <div class="container text-center mt-3">
                  <button class="btn btn-primary view-details-btn" data-detail='${JSON.stringify(
                    detail
                  )}'>View Details</button>
              </div>
          `;
  ...
  function showSweetAlert(detail) {
    Swal.fire({
      title: detail.place,
      html: `
  ...
  if (document.querySelector("#pages") && document.querySelector("#gallery")) {
    renderPagination(details.length);
    renderDestinations();
  } else {
    console.error(
      "Pagination or Gallery container not found during initial load"
    );
  }
});
